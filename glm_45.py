...
df_p = df[(df[ANGLE_N1_COL] == BOUNDARY_ANGLE) & (df[HITMISS_N1_COL] == 1)].copy()
...
df_p['angle_hit_n-1'] = df_p['angle_n-1']*df_p['hit_n-1'] # angle of the previous trial if it was a hit, 0 if it was a miss
df_p['angle_miss_n-1'] = df_p['angle_n-1']*(1 - df_p['hit_n-1']) # angle of the previous trial if it was a miss, 0 if it was a hit

# 4 models with increasing complexity: now it is a bit hardcoded but we can easily make it more flexible later
features_0 = ['angle']
features_1 = ['angle', 'action_n-1', ]
features_2 = ['angle', 'success_n-1', 'failure_n-1']
features_3 = ['angle', 'success_n-1', 'failure_n-1', 'angle_n-1']
features_5 =[ 'angle', 'angle_n-1']
features_6 = ['angle', 'angle_hit_n-1', 'angle_miss_n-1']
features_7 = ['angle', 'angle_hit_n-1', 'angle_miss_n-1', 'action_n-1']
...
models = [features_0, features_1, features_2, features_3, features_5, features_6, features_7]
results = []
for model in models:
...
# ---- PLOTS: single heatmap per model with betas + significance asterisks ----
for i, (df_model, result) in enumerate(zip(dfs, results)):
    
    # 1. compute bootstrap p-values (use 1000 for final, 5 for testing)
    pval_df = an.bootstrap_pvalues(result, n_iterations=1000)
    
    # 2. single combined heatmap
    an.plot_bootstrap_pvalues(
        beta_df=df_model,
        pval_df=pval_df,
        model_label=f"Model {i}",
        beta_cols=beta_cols          # shared x-axis across models
    )
